"""
omnivoice_narrator.py
Narração TTS usando OmniVoice (k2-fsa/OmniVoice).
Suporta 600+ idiomas incluindo português nativo.
Clonagem de voz zero-shot via ref_audio + ref_text.
Roda em CPU (device_map="cpu").
"""
...
logging.basicConfig(level=logging.INFO, format="[OmniVoiceNarrator] %(message)s")
...
class OmniVoiceNarrator:
    """
    Narrador usando OmniVoice TTS com clonagem de voz zero-shot.
    ...
    def _get_model(self):
        ...
        try:
            ...
            log.info("Carregando OmniVoice (primeira vez: baixa modelos)...")
            self._model = OmniVoice.from_pretrained(
                "k2-fsa/OmniVoice",
                device_map="cpu",
                dtype=torch.float32,
            )
            log.info("OmniVoice carregado!")
            return self._model
        except ImportError:
            raise ImportError(
                "OmniVoice não instalado. Execute:\n"
    ...
    def _sintetizar_sentenca(self, model, sentenca: str, output_path: str):
        ...
        kwargs = {"text": sentenca}
        if self.modo_clonagem:
            kwargs["ref_audio"] = self.voice_sample
            kwargs["ref_text"] = self.ref_text

        audio = model.generate(**kwargs)
        sf.write(output_path, audio[0], _SAMPLE_RATE)
